...
void keyboard_pad_handler::process()
{
	...
	for (uint i = 0; i < m_bindings.size(); i++)
	{
		...
		// Taiko hit capture: detected here at raw input-poll time so a hit occurring
		// between two USIO polls is never lost, then flagged for translate_input_taiko()
		// to consume (one hit per lane per call).
